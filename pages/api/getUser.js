...
export default async function handler(req, res) {
  if (req.method === 'GET') { // Verifica se o método da requisição é GET
    try {
      const dados = await prisma.aluno.findMany(); // Busca todos os usuários no banco de dados utilizando o Prisma
      res.status(200).json({ dados }); // Responde com um status 200 e um objeto JSON contendo os usuários obtidos
    } catch (error) {
